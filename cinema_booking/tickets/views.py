...
def seat_sel_view(request, showtime_id):
    try:
        ...
        # Get sold seats (seats with paid tickets)
        sold_seats = Ticket.objects.filter(
            showtime=showtime,
            status='paid'
        ).values_list('seats__id', flat=True)
...
def admin_dashboard(request):
    ...
    # Rạp đang hoạt động
    active_theaters = Theater.objects.filter(status='active').count()

    # Các biến tổng hợp khác
    total_users = User.objects.count()
    total_movies = Movie.objects.count()
    total_theaters = Theater.objects.count()
    today_ = today
    tickets_in_range = Ticket.objects.filter(
        purchase_date__date__range=[today_ - timedelta(days=30), today_]
    )
...
def get_recent_activities():
    ...
    for ticket in recent_tickets:
        activities.append({
            ...
            'user': ticket.user,  # Trả về object User
...
def manage_showtimes(request):
    ...
    showtimes = Showtime.objects.select_related('movie', 'room', 'room__theater').order_by('-show_date', '-start_time')
